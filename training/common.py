...
def get_metric_report_str(metric_report: dict) -> str:
    """
    Get a string representation of a metric report
    :param 
        metric_report: A dictionary of metrics
    :return 
        s: A string representation of the metrics
    """
    s = ""
...
def do_evaluation(trainer, dataset, split, parameters):
    """
    Evaluate a model on a dataset and log the results
    :param 
        trainer: The HuggingFace trainer object
        dataset: The dataset to evaluate on
        split: The split of the dataset
        parameters: A dictionary of parameters
    """
    special_logging = parameters['logger']
    with torch.no_grad():
...
def check_data_args(data_args, parameters):
    logger = parameters['logger']
    if not (data_args.train_file is None == data_args.validation_file is None):
        log_error(logger, "Cannot use --train_file without --validation_file")

    if data_args.test_file is None:
        logger.info(f"No test_file provided, will use part of validation set for test metrics")
    
    if data_args.data_file is not None:
        if data_args.train_file is not None:
            log_error(logger, "Cannot use --data_file with --train_file. Choose only one argument to provide")

    data_files = [data_args.test_file]

    if data_args.train_file is not None:
        data_files.append(data_args.train_file)
        data_files.append(data_args.validation_file)
    else:
        data_files.append(data_args.data_file)

    # TODO: Consider how to handle text in here.
    for file in data_files:
        if file.split(".")[-1] != "csv":
            log_error(logger, f"Data file {file} must be a csv")    


def common_setup(model_args, data_args, training_args, parameters):
    training_args.do_train = True
    training_args.do_eval = True
    training_args.predict_with_generate = True  # TODO: I forgot what this does
    ...
    if data_args.data_file is not None:
        ...
        df = df.sample(frac=1, random_state=parameters["random_seed"]).reset_index(drop=True)
    ...
    if data_args.input_column_name is not None:
        ...
        for name in ["input", "text", "prompt", "sentence"]:
